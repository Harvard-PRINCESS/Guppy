...
#ifndef ARCH_MIPS_BARRELFISH_KPI_DISPATCHER_SHARED_ARCH_H
#define ARCH_MIPS_BARRELFISH_KPI_DISPATCHER_SHARED_ARCH_H

#include <target/mips/barrelfish_kpi/dispatcher_shared_target.h>
                                             ...
                                             uintptr_t rip)
{
    ...
    /* one crit_pc pair */
    struct dispatcher_shared_mips *dispmips =
        get_dispatcher_shared_mips(handle);
    return disp->disabled ||
        (dispmips->crit_pc_low <= rip && rip < dispmips->crit_pc_high);
}

static inline arch_registers_state_t*
dispatcher_get_enabled_save_area(dispatcher_handle_t handle)
{
    return &((struct dispatcher_shared_mips *)handle)->enabled_save_area;
}

static inline arch_registers_state_t*
dispatcher_get_disabled_save_area(dispatcher_handle_t handle)
{
    return &((struct dispatcher_shared_mips *)handle)->disabled_save_area;
}

static inline arch_registers_state_t*
dispatcher_get_trap_save_area(dispatcher_handle_t handle)
{
    return &((struct dispatcher_shared_mips *)handle)->trap_save_area;
}

#endif // ARCH_MIPS_BARRELFISH_KPI_DISPATCHER_SHARED_ARCH_H
